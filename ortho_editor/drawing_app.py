...
class DrawingApp(QMainWindow):
    ...
    def connect_actions(self):
        ...
        # File actions
        self.ui.open_action.triggered.connect(self.open_directory)
        self.ui.save_action.triggered.connect(self.save_image)

        # Tool buttons
        self.ui.pencil_button.clicked.connect(lambda: self.select_tool("Pencil"))
        self.ui.eraser_button.clicked.connect(lambda: self.select_tool("Eraser"))
        self.ui.line_button.clicked.connect(lambda: self.select_tool("Line"))
        self.ui.fill_button.clicked.connect(lambda: self.select_tool("Fill"))
        ...
        self.ui.smooth_thin_smooth_action.triggered.connect(
            self.smooth_thin_smooth_image
        )
    ...
    def select_tool(self, tool):
        ...
        buttons = {
            "Pencil": self.ui.pencil_button,
            "Eraser": self.ui.eraser_button,
            "Line": self.ui.line_button,
            "Fill": self.ui.fill_button,
        }

        # Set checked state for buttons
        for t, button in buttons.items():
    ...
    def smooth_image(self):
        ...
        if self.image:
            ...
            self.image = ImageProcessor.smooth_image(
                self.image, self.graphics_view.pen_color
            )
            self.update_scene()
    ...
    def thin_image(self):
        ...
        if self.image:
            self.save_undo_state()
            self.image = ImageProcessor.thin_image(
                self.image, self.graphics_view.pen_color
            )
            self.update_scene()
    ...
    def smooth_thin_smooth_image(self):
        ...
        if self.image:
            self.save_undo_state()
            self.image = ImageProcessor.smooth_thin_smooth_image(
                self.image, self.graphics_view.pen_color
            )
            self.update_scene()
    ...
    def open_directory(self):
        """Open a directory dialog and reinitialize app with selected directory"""
        ...
        if directory:
            ...
            current_settings = {
                ...
                "opacity": self.ui.opacity_slider.value(),
            }
            ...
            os.makedirs(os.path.join(directory, "inputs"), exist_ok=True)
            os.makedirs(os.path.join(directory, "targets"), exist_ok=True)

            # Clear current image and scene
            self.image = None
            self.background_image = None
            ...
            # Restore settings
            self.graphics_view.pen_size = current_settings["size"]
            self.graphics_view.pen_color = current_settings["color"]
            self.select_tool(current_settings["tool"])
            self.ui.opacity_slider.setValue(current_settings["opacity"])
